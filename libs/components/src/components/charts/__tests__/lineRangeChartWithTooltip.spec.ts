...
import LineRangeChartWithTooltip from '@comp/components/charts/line-range/LineRangeChartWithTooltip.vue';

const getLineRangeChartWithTooltip = () =>
  shallowMount(LineRangeChartWithTooltip, {
    props: {
...
describe('Компонент LineRangeChartWithTooltip', () => {
  ...
  test('При ширине экрана больше или равной lg всплывающая подсказка будет попапом', () => {
    ...
    expect(wrapper.findComponent(ChartTooltip).exists()).toBeTruthy();
  });
  ...
  test('Проп chartData передается в дочерний компонент', () => {
    ...
    const wrapper = getLineRangeChartWithTooltip();
    expect(wrapper.findComponent(LineRangeChart).props().chartData).toStrictEqual(propChartData);
  });
});
