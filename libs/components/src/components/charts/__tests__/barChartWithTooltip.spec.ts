...
import BarChartWithTooltip from '@comp/components/charts/bar-chart/BarChartWithTooltip.vue';

const getBarChartWithTooltip = () =>
  shallowMount(BarChartWithTooltip, {
    props: {
...
describe('Компонент BarChartWithTooltip', () => {
  ...
  test('Проп chartData передается в дочерний компонент', () => {
    ...
    const wrapper = getBarChartWithTooltip();
    expect(wrapper.findComponent(BarChart).props().chartData).toStrictEqual(propChartData);
  });
});
